...
- Numbered section markers (`01/02/03`) are allowed ONLY because the nav and case studies are genuinely sequential — do not add numbering elsewhere decoratively
...
2. **Engineering Case Studies** — 2–4 projects max, array-driven component. Each has: index tag (`STUDY_01`...), project name + one-line problem statement, role, stack as JetBrains Mono tags, blueprint-style architecture diagram placeholder (monochrome + green accent lines), 2–3 quantified outcomes, links (code/demo/write-up)
...
- Border color shift (`neutral-800` → accent green) on component hover — case study cards and table rows
...
- Every case study/table entry driven by a typed data structure (array of objects), not hardcoded JSX per item
